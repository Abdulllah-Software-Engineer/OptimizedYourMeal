...
interface RootState {
  meal: {
    selectedItemsWeekThree: Recipe[];
  };
}

const WeekThree: FC = () => {
  const dispatch = useDispatch();
  const selectedItemsWeekThree = useSelector(
    (state: RootState) => state.meal.selectedItemsWeekThree
  );

  const handleDelete = (id: number) => {
    dispatch(unselectItem({ week: "Week 3", id }));
  };

  return (
    <div className="container-wrapper grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-14 py-10">
      {selectedItemsWeekThree.map((recipe) => (
        <div
          key={recipe.id}
