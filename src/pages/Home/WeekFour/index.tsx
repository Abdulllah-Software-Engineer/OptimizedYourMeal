...
interface RootState {
  meal: {
    selectedItemsWeekFour: Recipe[];
  };
}

const WeekFour: FC = () => {
  const dispatch = useDispatch();
  const selectedItemsWeekFour = useSelector(
    (state: RootState) => state.meal.selectedItemsWeekFour
  );

  const handleDelete = (id: number) => {
    dispatch(unselectItem({ week: "Week 4", id }));
  };

  return (
    <div className="container-wrapper grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-14 py-10">
      {selectedItemsWeekFour.map((recipe) => (
        <div
          key={recipe.id}
