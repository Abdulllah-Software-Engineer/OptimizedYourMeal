...
interface RootState {
  meal: {
    selectedItemsWeekTwo: Recipe[];
  };
}

const WeekTwo: FC = () => {
  const dispatch = useDispatch();
  const selectedItemsWeekTwo = useSelector(
    (state: RootState) => state.meal.selectedItemsWeekTwo
  );

  const handleDelete = (id: number) => {
    dispatch(unselectItem({ week: "Week 2", id }));
  };

  return (
    <div className="container-wrapper grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-14 py-10">
      {selectedItemsWeekTwo.map((recipe) => (
        <div
          key={recipe.id}
